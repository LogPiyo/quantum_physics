...
def adia_eng(t: complex, Ham: Callable[..., complex], ut: bool = False, real: bool = False, F: float | None = None) -> complex | float:
...
def eig_vec(t: float, Ham: Callable[..., float], s: str) -> np.ndarray:
    """
    eigenvector
...
def TLZ_theoretical(v: float, F: float, m: float, k: float) -> float:
    TLZ: float = -np.pi * (m + k * v * F / 4)**2 / (abs(v) * abs(F))
    return np.exp(TLZ)


def func_psi_module(t: float, Ham: Callable[..., float], var: list[float], h: float = 1) -> list[float]:
    """
    state vector
